...
CACHE_DIR = Path(user_cache_dir("pbcmd")) / "cpush/cache"
ROOT = Path("/")
...
def do_cpush(lconfig: LocalConfig, remote: str, project_dir: Path):
    click.secho(f"Using remote: {remote}", fg="yellow")

    remote_dir = lconfig.remotes[remote]
    backup_dir = CACHE_DIR / lconfig.project / remote / "remote_backup"
    borg_repo = CACHE_DIR / lconfig.project / "borg_repo"
